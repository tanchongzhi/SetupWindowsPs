...
$interfaceStates = $interfaceStatesRaw | ForEach-Object { $_.Trim() } | Where-Object -FilterScript { $_ }
...
$name = $name.Trim()
$isValidName = $interfaceNames | Where-Object -FilterScript { $_ -eq $name }
...
New-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\services\Dnscache\Parameters\' -Name AddrConfigControl -Value 0 -PropertyType DWord -Force | Out-Null
New-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\services\TCPIP6\Parameters\' -Name DisabledComponents -Value 0 -PropertyType DWord -Force | Out-Null
