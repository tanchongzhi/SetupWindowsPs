...
if ([System.Environment]::OSVersion.Version.Major -eq 6) {
    ...
    Write-Host 'System : Language Bar : Show additional icons in taskbar : Disabled'
    $null = New-ItemProperty -Path $Key -Name ExtraIconsOnMinimized -Value 0 -PropertyType DWord -Force

    Write-Host 'System : Language Bar : Show text labels : Enabled'
    $null = New-ItemProperty -Path $Key -Name Label -Value 1 -PropertyType DWord -Force

    Write-Host 'System : Language Bar : Docked in the taskbar : Enabled'
    ...
    Write-Host 'System : Language Bar : Auto Adjust : Enabled'
